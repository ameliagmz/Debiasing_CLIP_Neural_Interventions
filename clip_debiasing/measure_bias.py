...
def measure_bias_no_batches(model, img_preproc: Callable, tokenizer: Callable,
                          attribute="gender", dataset="fairface", mode='test'):
    # load dataset
    if dataset == "fairface":
        ds = FairFace(mode=mode, iat_type=attribute, transforms=img_preproc)
    elif dataset == "facet":
    ...
    # --- compute image embeddings sample by sample (no batches) ---
    image_embeddings = []
    image_labels = []

    for idx in tqdm(range(len(ds)), desc="Embedding images (no batch)"):
        sample = ds[idx]   # dictionary with "img" and "iat_label"
        with torch.no_grad():
            # add batch dimension before encoding
            img_emb = model.encode_image(sample["img"].unsqueeze(0).to(device))
            image_embeddings.append(img_emb.cpu())
        image_labels.append(sample["iat_label"])

    image_embeddings = torch.cat(image_embeddings, dim=0).to(device)
    labels_list = np.array(image_labels)

    # --- compute text embeddings (still batched, since small set) ---
    prompts_embeddings = get_prompt_embeddings(model, tokenizer, device, prompts)

    # --- run evaluations ---
    result = {}
    for evaluation in evals:
        result[evaluation] = eval_ranking(
            labels_list,
            image_embeddings,
            prompts_embeddings,
            evaluation,

...
import argparse
import clip
# from zhang2024.VL_Debiasing.clip_debiasing.models.model_vl_debiasing import DebiasedCLIP
...
from params import BASE_DIR, EXP_DIR
...
def evaluate_normalized_entropy(image_embeddings, text_embeddings, dataset, taxonomy, k=100):
    results_race = {}
    results_gender = {}
    similarities = {}
...
def evaluate_CASC(similarities, dataset, taxonomy, text_embeddings):
    """
...
def run_eval_So_B_IT(model, preprocess, device, results_file, dataset, taxonomy):
    ...
    race_results, gender_results, similarities = evaluate_normalized_entropy(image_embeddings, text_embeddings, dataset, taxonomy)
    with open(os.path.join(EXP_DIR, results_file), "a") as file:
        for category in taxonomy.keys():
            file.write(f"Category: {category}, Race Bias (Normalized Entropy): {race_results[category]:.3f}\n")
            file.write(f"Category: {category}, Gender Bias (Normalized Entropy): {gender_results[category]:.3f}\n")
    
    # Evaluate C-ASC
    casc_results = evaluate_CASC(similarities, dataset, taxonomy, text_embeddings)
    with open(os.path.join(EXP_DIR, results_file), "a") as file:
        file.write("\n")
...
def eval_So_B_IT(model, preprocess, model_type, backbone, test_attribute, interv, FFcrop):
    with open(os.path.join(BASE_DIR, 'So-B-IT_taxonomy.json')) as f:
    ...
    dataset = load_dataset('HuggingFaceM4/FairFace', FFcrop, split='validation')

    # device setup
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    results_file = "So_B_IT_results_White.txt"

    with open(os.path.join(EXP_DIR, results_file), "a") as file:  
        file.write(f"MODEL: {model_type}, BACKBONE: {backbone}, TEST ATTRIBUTE: {test_attribute}, FF CROP: {FFcrop}\n\n")


    if model_type == 'original':
        model, preprocess = clip.load(backbone, device=device)
        run_eval_So_B_IT(model, preprocess, device, results_file, dataset, taxonomy)
        ...
        preprocess = model.preprocess
        model.load_state_dict(torch.load("vit_b_16_fairface_gender_renamed2.pth")) # change the path
        run_eval_So_B_IT(model, preprocess, device, results_file, dataset, taxonomy)
